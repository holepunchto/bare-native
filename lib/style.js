...
const TARGETS = new Set(['auto', 'none'])

// Whether a box is something a press can land on. CSS has more values for SVG
// and React Native two more for a box that answers for itself but not for its
// children; both of those would be hit testing written here rather than asked
// for, on all five, so what crosses is the pair every toolkit has a word for.
function targets(value) {
  if (value === undefined) return true

  if (!TARGETS.has(value)) {
    throw new TypeError(`Unknown value '${value}' for style property 'pointerEvents'`)
  }
...
// A style may be written as a list, which is how one is composed out of others
// and the only way a caller says a conditional one. Later entries win, and an
// entry that is nothing is skipped, so a style can be written beside the test
// that decides whether it applies.
function flatten(style, into = {}) {
  if (style === null || style === undefined || style === false) return into
  ...
  for (const name in style) into[name] = style[name]
