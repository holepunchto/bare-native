...
// inside it.
const RUN = new Set(['color', 'fontFamily', 'fontSize', 'fontWeight', 'fontStyle'])
...
const TEXT = new Set([...PAINT, ...RUN, 'textAlign'])
...
const KNOWN = new Set([...PAINT, ...RUN, ...IMAGE, ...TEXT])
...
function apply(node, style, paint, accepted = PAINT, env = ENV, previous = null) {
  ...
  for (const name in style) {
    ...
    if (accepted.has(name)) paint[name] = dynamic(value) ? number(value, env) : value
...
module.exports = { apply, color, fit, italic, weight, ENV, IMAGE, LAYOUT, PAINT, RUN, TEXT }
