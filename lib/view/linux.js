...
module.exports = class NativeView extends NativeNode {
  ...
  _paint(properties) {
    ...
    for (const name in properties) {
      ...
      switch (name) {
        case 'opacity':
          this._native.opacity = value === undefined ? 1 : value
          break
        case 'overflow':
          this._native.overflow = value === 'hidden' ? OVERFLOW_HIDDEN : OVERFLOW_VISIBLE
          break
        case 'backgroundColor':
          this._declarations['background-color'] = value === undefined ? null : rgba(value)
          restyle = true
          break
        case 'borderRadius':
          this._declarations['border-radius'] = value === undefined ? null : `${value}px`
          restyle = true
          break
        case 'borderColor':
          ...
          break
        case 'borderWidth':
          this._borderWidth = value === undefined ? 0 : value
          restyle = true
          break
      }
    }
  ...
  _restyle() {
    ...
    if (this._borderWidth > 0 && this._borderColor !== undefined) {
