...
const { Control, FocusManager, PasswordBox, SolidColorBrush, TextBox } = WinUI

const { INPUT_SCOPE_NAME, TEXT_WRAPPING } = TextBox
const { PASSWORD_REVEAL_MODE } = PasswordBox
...
module.exports = class NativeTextInput extends NativeTextInputNode {
  ...
  _focus() {
    this._native.focus(Control.FOCUS_STATE.PROGRAMMATIC)
  }

  // Focus is given and never taken away here, so letting go is moving it on.
  _blur() {
    FocusManager.tryMoveFocus(this._native, FocusManager.FOCUS_NAVIGATION_DIRECTION.NEXT)
  }
