...
module.exports = class NativeTextInput extends NativeTextInputNode {
  constructor(opts) {
    ...
    // An input centres its one line in whatever box it is given and a text
    // area starts at the top, which is what the web does. A control puts what
    // its template holds where this says.
    this._native.verticalContentAlignment = this._multiline
      ? WinUIControl.VERTICAL_ALIGNMENT.TOP
      : WinUIControl.VERTICAL_ALIGNMENT.CENTER
  ...
  // whatever is measured here. All three are put back rather than left for the
  // flush, because a pass that measures a node does not always place it: a
  // layout that came out the same as the last one is not a new one, and a node
  ...
  _onmeasure(width, widthMode, height, heightMode) {
    ...
    const box = { width: this._native.width, height: this._native.height }
    ...
    this._native.width = box.width
    this._native.height = box.height
  ...
  // the template it already has, and the window dies when XAML draws it. A
  // shadow is what that costs, since one is cast by a visual kept behind the
  // content and there is nowhere here to keep it.
  _paint(properties) {
  ...
  // A control pads what its template holds, which is what the padding means
  // here.
  _resized(width, height) {
    const { top, right, bottom, left } = this._padding

    this._native.padding = { left, top, right, bottom }
  }
