...
module.exports = class NativeText extends NativeTextNode {
  ...
  _onmeasure(width, widthMode, height, heightMode) {
    ...
    const measured = this._native.desiredSize
  ...
  _apply() {
    ...
    for (const run of this._runs) {
      ...
      inline.textDecorations =
        (lines.includes('underline') ? Run.TEXT_DECORATIONS.UNDERLINE : 0) |
        (lines.includes('line-through') ? Run.TEXT_DECORATIONS.STRIKETHROUGH : 0)
      inline.fontFamily =
  ...
  _paint(properties) {
    ...
    if ('lineHeight' in properties) {
      ...
      this._native.lineHeight = lineHeight === undefined ? 0 : lineHeight
