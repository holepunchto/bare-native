...
module.exports = class NativeText extends NativeTextNode {
  constructor(text = '') {
    ...
    this._text = new AppKitTextField({ x: 0, y: 0, width: 0, height: 0 })
  ...
  _apply() {
    ...
    this._text.attributedStringValue = this._attributed
  ...
  // The count is the control's and the cut is the paragraph's, so the string
  // is built again to carry it.
  _truncated(lines, mode) {
    this._text.maximumNumberOfLines = lines

    // A paragraph style that truncates stops the text wrapping at all, and the
    // cell's own break mode is overridden by the style the string carries. So
    // the string wraps and the field marks the line it stopped on, which is
    // the one thing here that is neither.
    this._text.truncatesLastVisibleLine = lines !== 0 && mode === 'tail'
  }
